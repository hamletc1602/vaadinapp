...
public class MockAssetService implements AssetService {
    ...
    private static MockAssetService instance;
    private static final Logger LOGGER = Logger.getLogger(MockAssetService.class.getName());

    private LoadData data;
    
    private MockAssetService() {
    }

    /**
     * @return a reference to an example facade for Asset objects.
     */
    public static MockAssetService getInstance() {
        if (instance == null) {
            instance = new MockAssetService();
        }
        return instance;
    }
